...
estados.forEach((estado, index) => console.log(`${index + 1} - ${estado.toUpperCase()}`));
...
pessoasEstudantes.forEach((student) => {
  let mediaFinal = (student.avaliacao + student.projeto + student.trabalhoGrupo) / 3;
  if (mediaFinal >= 7) {
    console.log({nome: student.nome, media: parseFloat(mediaFinal).toPrecision(2)});
  }
});
